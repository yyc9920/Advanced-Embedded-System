...
extern unsigned char achroimx_bmp_rgb565_1024x600[];
extern int achroimx_bmp_rgb565_1024x600_size;
...
//1000000 / pixclock = 1000000 / 22222 = 45MHz pixel clock.

static struct fb_videomode lvds_wsvga = {
    "WSVGA", 60, 1024, 600, 22222, 220, 40, 21, 7, 60, 10,
    FB_SYNC_EXT,
    FB_VMODE_NONINTERLACED,
    0,
};
...
#define GPIO_LVDS_PWM3 IMX_GPIO_NR(2, 9)
...
#ifdef ACHROIMX6
    mxc_iomux_v3_setup_pad(MX6X_IOMUX(PAD_DISP0_DAT19__GPIO_5_13));
	gpio_direction_output(GPIO_LCD_BIAS_EN, 1);

    mxc_iomux_v3_setup_pad(MX6X_IOMUX(PAD_SD4_DAT1__GPIO_2_9));
	gpio_direction_output(GPIO_LVDS_PWM3, 1);

    mxc_iomux_v3_setup_pad(MX6X_IOMUX(PAD_DISP0_DAT12__GPIO_5_6));
...
#else
	ret = ipuv3_fb_init(&lvds_wsvga, di, IPU_PIX_FMT_RGB666,
			DI_PCLK_LDB, 65000000);
#endif
	if (ret)
...
#else
	panel_info.vl_col = lvds_wsvga.xres;
	panel_info.vl_row = lvds_wsvga.yres;
#endif
...
void setup_splash_image(void)
{
	...
	if (s != NULL) {
		...
		addr = ioremap_nocache(iomem_to_phys(addr),
				achroimx_bmp_rgb565_1024x600_size);
#endif
		memcpy((char *)addr, (char *)achroimx_bmp_rgb565_1024x600,
				achroimx_bmp_rgb565_1024x600_size);
	}
}
#endif
...
int checkboard(void)
{
	printf("Board: Achro %s Board: 0x%x [",
	mx6_chip_name(),
	fsl_system_rev);
