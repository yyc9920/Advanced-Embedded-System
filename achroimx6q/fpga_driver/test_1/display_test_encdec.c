...
void *fpgaThread(void *data)
{
	...
	int data1[5] = {0}, i, otp_int, dip_int;
	...
	{/*
		switch(section){
			case password_section: 
				...
				{
					data1[i] = push_switch();
					sprintf(data_str, "%d%d%d%d%d", data1[0], data1[1], data1[2], data1[3], data1[4]);
					fnd(data_str);
				}
				for(i = 0; i < 4; i++)
					sprintf(value, "%d%d%d%d",data1[0], data1[1], data1[2], data1[3]);
				printf("%s\n", value);
				reset_fnd();
